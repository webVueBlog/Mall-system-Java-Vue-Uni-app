...
#jar包文件名称
APP_NAME=$APP_PATH/da-admin.jar

#日志文件名称
LOG_FILE=$APP_PATH/da_out.log

#启动环境
# 如果需要配置数据和redis，请在 application-prod.yml中修改, 用jar命令修改即可
...
# 启动jar包，指向日志文件，2>&1 & 表示打开或指向同一个日志文件  --spring.profiles.active=prod 启动 prod环境
  ...
  while [ -f $LOG_FILE ]
  do
      success=`grep "Started dadaAdminApplication in " $LOG_FILE`
      if [[ "$success" != "" ]]
      then
#          echo "dada start ........."
          break
